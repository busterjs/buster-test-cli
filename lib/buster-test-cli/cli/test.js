...
module.exports = buster.extend(buster.create(require("buster-cli")), {
    ...
    onRun: function () {
        ...
        this.onConfig(function (err) {
            if (err) {
                this.logger.e(err.message);
                if (err.stack) this.logger.e(err.stack);
            } else {
                for (var configName in this.config.value) {
                    var config = this.config.value[configName];
                    this.logger.info("Running tests:", configName);
                    this.logger.debug("Loading:", config.load);
