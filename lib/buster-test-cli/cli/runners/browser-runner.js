...
module.exports = {
    run: function (config, options, done) {
        ...
        this.addPreSessionHook(config, function () { client.abort(); }, options);
    ...
    addPreSessionHook: function (config, onFail, options) {
        ...
        analyzer.on("fail", onFail);
        config.runExtensionHook("beforeRun", config, analyzer);
        return analyzer;
    },
