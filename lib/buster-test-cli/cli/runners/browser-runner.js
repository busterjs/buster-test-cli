...
module.exports = {
    ...
    runSession: function (session) {
        ...
        if (!opt.clients || opt.clients.length == 0) {
            return runComplete.call(this, session, opt.clients);
        }
        ...
        remoteRunner.on("suite:end", runComplete.bind(this, session, opt.clients));
    },
...
function runComplete(session, clients) {
    var callback = this.callback || function () {};
    var logger = this.logger;

    if (!clients || clients.length == 0) {
        logger.warn("No clients connected, nothing to do.");
        logger.warn("Capture browsers at " + this.server.href + " and try again.");
