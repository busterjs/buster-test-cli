...
module.exports = {
    ...
    runSession: function (session) {
        ...
        var runnerConfig = config(opt, ["autoRun", "filters"]);
        runnerConfig.failOnNoAssertions = !!opt.failOnNoAssertions;
        var remoteRunner = bRemoteRunner.create(session.messagingClient, session.clients, runnerConfig);
...
function config(options, keys) {
    return keys.reduce(function (opts, key) {
...
function createReporter(runner, session, options) {
    ...
    var reporter = bReporters[options.reporter || "dots"].create({
        io: ioStream,
