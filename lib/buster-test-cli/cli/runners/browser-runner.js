...
module.exports = {
    run: function (config, options) {
        ...
        client.createSession(config.resourceSet).then(
    ...
    runSession: function (session) {
        ...
        var runnerConfig = { failOnNoAssertions: !!opt.failOnNoAssertions };
        if (opt.hasOwnProperty("autoRun")) runnerConfig.autoRun = opt.autoRun;
        var remoteRunner = bRemoteRunner.create(session.messagingClient, runnerConfig);
...
function createReporter(runner, session, options) {
    ...
    if (!options.reporter || options.reporter == "xUnitConsole") {
    ...
    var reporter = bReporters[options.reporter || "xUnitConsole"].create({
        io: ioStream,
