...
function clientToString() {
    var str = [this.browser, this.version, this.platform].join(" ");
...
module.exports = buster.extend(buster.create(buster.eventEmitter), {
    create: function (emitter, clients, config) {
        var runner = Object.create(this);
        runner.console = runner;
        runner.config = config || {};
        runner.clientIds = (clients || []).map(property("id"));
        runner.clientCount = runner.clientIds.length;
    ...
    clientReady: function (e) {
        if (this.clientIds.indexOf(e.clientId) < 0) return;
