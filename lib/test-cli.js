...
function addCLIArgs(cli) {
    ...
    cli.opt(["-d", "--deterministic"], {
        description: "Do not run tests in random order (not recommended " +
            "except for debugging inter-test dependency bugs)."
    });

    cli.opt(["--fail-on-focus"], {
        description: "Fail the whole suite if any focus rockets (=>) found."
    });

    cli.opt(["-S", "--random-seed"], {
        description: "Randomize tests using this seed. Useful to reproduce " +
            "failures that result from inter-test dependencies that are only " +
            "triggered for certain orderings. Most reporters will tell you " +
            "what seed is being used so you can recreate the exact same run.",
        hasValue: true
    });
...
function prepareOptions(prefs, args) {
    return _.extend({
        reporter: process.env["BUSTER_REPORTER"] || args["--reporter"].value,
        ...
        captureConsole: !cli.pref(prefs, args["--release-console"],
            "test.releaseConsole"),
        staticResourcesPath: args["--static-paths"].isSet,
        logPassedMessages: !cli.pref(prefs, args["--quiet-log"],
            "test.quietLog"),
        filterStacks: !cli.pref(prefs, args["--full-stacks"],
            "test.fullStacks"),
        random: !cli.pref(prefs, args["--deterministic"], "test.deterministic"),
        randomSeed: cli.pref(prefs, args["--random-seed"], "test.randomSeed"),
        allowFocusMode: !args["--fail-on-focus"].isSet
        ...
        this.cli.parseArgs(cliArgs, function (err, options) {
            try {
                ...
                this.cli.loadConfig(options, function (err, groups) {
                    ...
                    // var runners = loadRunners(groups,
                    //     prepareOptions(this.preferences, options));
                    // var runner = mr.createMultiRunner(runners);
                    // runner.start.then(partial(callback, null), callback);
