...
args.validators.reporter = function (errMsg) {
    return function (arg) {
        ...
        try {
            deferred.resolve(bTest.reporters.load(arg.value));
        } catch (e) {
            deferred.reject(
...
function addCLIArgs(cli) {
    return {
        ...
        quietLog: cli.opt(
            "-q",
            "--quiet-log",
            "Don't print log messages from passing tests"
        ),
        ...
        fullStacks: cli.opt(
            "-f",
            "--full-stacks",
            "Full, unfiltered stack traces for errors. " +
...
function prepareOptions(prefs, args) {
    return B.extend({
        ...
        staticResourcesPath: args.staticResourcesPath.isSet,
        logPassedMessages: !cli.pref(prefs, args.quietLog, "test.quietLog")
...
function configureStackFilters(prefs, args) {
    if (cli.pref(prefs, args.fullStacks, "test.fullStacks")) {
...
module.exports = {
    ...
    run: function (cliArgs, callback) {
        ...
        this.cli.parseArgs(cliArgs, function (err) {
            ...
            configureStackFilters(this.prefsink, args);
            this.cli.loadConfig(B.bind(this, function (err, groups) {
                ...
                this.runConfigGroups(
                    groups, prepareOptions(this.prefsink, args), callback
                );
            }));
        }.bind(this));
    },
    ...
    summary: function (runs) {
        ...
        for (i = 0, l = runs.length; i < l; ++i) {
            if (runs[i].error) {
                err = runs[i].error;
                return this.logger.error(err.stack || err.message);
            }
        }
    },
