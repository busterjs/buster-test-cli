...
var testRun = {
    ...
    runTests: function (session, callback) {
        ...
        var runner = remoteRunner.create(session, session.slaves);
    ...
    start: function () {
        var client = createServerClient(this.options.server);
        this.config.resolve().then(this.startSession(client, function (err, session) {
            ...
            this.runTests(session, B.bind(this, "done"));
        }.bind(this)));
    }
};
...
module.exports = {
    ...
    run: function (config, options, done) {
        this.run = testRun.create(config, options || {}, this.logger, done);
        this.run.cacheable = this.cacheable;
        this.run.start();
        return this.run;
    },

    // TODO: The node runner should also return a runner object
    // from run(). That way the test-cli should set the cacheable
    // flag directly on the run, not the runner.
    set cacheable(cacheable) {
        this._cacheable = cacheable;
        if (this.run) { this.run.cacheable = cacheable; }
    },

    get cacheable() {
        return typeof this._cacheable === "boolean" ? this._cacheable : true;
    },

    // TODO: The node runner should also return a runner object
    // from run(). That way the test-cli should abort() the run,
    // not the runner
    abort: function (err) {
        if (this.run) { this.run.abort(err); }
    },
