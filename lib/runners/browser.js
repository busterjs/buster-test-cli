...
function terminalOptions(options, logger) {
    return {
...
module.exports = {
    ...
    runSession: function (session, options, done) {
        var formatter = terminal.create(terminalOptions(options, this.logger));
        logSessionLifeCycle(session, this.logger);
        listenForUncaughtExceptions(session, this.logger, formatter);
        done();
    },
