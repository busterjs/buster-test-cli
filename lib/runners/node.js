...
var testRun = {
    ...
    start: function () {
        this.configure();
        try {
            when.all([this.beforeRunHook(), this.config.resolve()]).
    ...
    runTests: function (rs, manifests) {
        ...
        if (this.cacheable) { writeManifest(cacheFile(this.config), manifests); }
...
module.exports = {
    ...
    create: function (options) {
        return B.extend(B.create(this), { _cacheable: options.cacheable });
    },
